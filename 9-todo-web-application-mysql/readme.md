...
Run com.spring.boot.web.SpringBootFirstWebApplication as a Java Application.
...
- `com.spring.boot.web.security.SecurityConfiguration` contains the in memory security credential configuration.
...
```
docker container run -p 8080:8080 --link=mysql -e RDS_HOSTNAME=mysql  lalitrnagpal/todo-web-application-mysql:0.0.1-SNAPSHOT
```

Using Custom Network

```
docker container run -p 8080:8080 --network=web-application-mysql-network -e RDS_HOSTNAME=mysql  09-lalitrnagpal/todo-web-application-mysql:0.0.1-SNAPSHOT
```
